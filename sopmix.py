...
def warmup(epoch, net, ema_net, optimizer, dataloader):
    net.train()
    ema_net.eval()
    ...
    for batch_idx, (inputs_w, inputs_s, labels, index) in enumerate(dataloader):
        ...
        labels_x = torch.zeros(len(labels), args.num_class).cuda().scatter_(1, labels.view(-1, 1), 1)
        optimizer.zero_grad()
        outputs = net(inputs_w)
        outputs2 = net(inputs_s)
        loss = train_loss(index, outputs, outputs2, labels_x, labels)
...
scheduler_trans = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_trans, args.num_epochs, eta_min=0.000001)
